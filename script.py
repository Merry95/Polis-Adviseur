...
#leeftijd = st.number_input("Wat is je leeftijd?", min_value=18, max_value=100, step=1)
email = st.text_input("Wat is je e-mailadres?")
woonSituatie = st.selectbox("Wat is je woonsituatie ?", ["Huurwoning", "Koopwoning", "inwonend"])
werkSituatie = st.selectbox("Wat is je werksituatie?", ["In loondienst", "zzp", "Werkloos"])
gezinssituatie = st.selectbox("Wat is je gezinssituatie ?", ["Alleen", "samenwonend", "kinderen","huisdieren"])
spaarGeld = st.selectbox("Heb je meer dan 25 000 euro aan spaargeld?", ["nee", "ja"])
if spaarGeld == "ja":
    spaarGeldGebruiken = st.selectbox("ben je bereid om dat uit te geven aan ongevallen??", ["ja", "nee"])
